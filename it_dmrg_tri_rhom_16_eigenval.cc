...
//	ITiVec C6_sym_mapping_table=C6_sym_mapping_table_32_site;
	...
	ITcVec COM_mom="1. 1.";
	COM_mom(0)=std::exp(Complex_i*1.*Pi/2.);
    COM_mom(1)=std::exp(-Complex_i*1.*Pi/2.);
    ...
    for (int p=0; p<4; p++) {
        for (int q=0; q<4; q++) {
            mom(0)=std::exp(Complex_i*(double) p*Pi/2.);
            mom(1)=std::exp(Complex_i*(double) q*Pi/2.);
            //cout<<mom<<endl;
	...
	////////cout<<"From rank="<<rank<<" Monte Carlo sym eigval: "<<symmetry_eigval_tj_MonteCarlo(desc,psi, C6_sym_mapping_table)<<endl;
