            ...
            for (int xsweep=1; xsweep<=nxsweep; xsweep++) {
                for (int X=0; X<nx; X++) {  //int X=0; X<nx; X++
                    ...
                    if (rank==root) {
                        ...
                        if (cholesky_flag==0) {
                            ///////
                            GeneralizedNonSymmetricEigenProblem(AF,BF,rrr,iii,beta,zzl,zzr);//generalized eigen problem
                            
                            for (int x=1; x<=2*IF.indices()[0].m(); x++) {
                                if (beta(x)!=0) {
                                    cout<<"("<<rrr(x)/beta(x)<<","<<iii(x)/beta(x)<<"),    ";
                                }
                            }
//                            //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//                            Vector _rrr(2*IF.indices()[0].m()), _iii(2*IF.indices()[0].m());
//                            for (int x=1; x<=2*IF.indices()[0].m(); x++) {
//                                if (beta(x)!=0) {
//                                    _rrr(x)=rrr(x)/beta(x);
//                                    _iii(x)=iii(x)/beta(x);
...
//                            //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                            ...
                            //----begin: find out the closest eigenvalue-------------
                            double energy_temp=energy;
                            energy=100000;
                            int signflag=0;
                            for (int x=1; x<=2*IF.indices()[0].m(); x++) {
                                if (iii(x)==0&&beta(x)!=0) {
                                    if (fabs(energy_temp-rrr(x)/beta(x))<energy) {
                                        energy=fabs(energy_temp-rrr(x)/beta(x));
                                        if (energy_temp<rrr(x)/beta(x)) {
                                            signflag=0;
                                        }
                                        else {
                                            signflag=1;
                                        }
                                        for (int y=1; y<=IF.indices()[0].m(); y++) {
                                            evector_real(y)=zzr(y,x);
                                            evector_imag(y)=zzr(y+IF.indices()[0].m(),x);
                                        }
                                    }
                                }
                            }
                            if (signflag==0) {
                                energy=energy_temp+energy;
                            }
                            else {
                                energy=energy_temp-energy;
                            }
                            //----end: find out the closest eigenvalue-------------
