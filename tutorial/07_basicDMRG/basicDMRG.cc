...
#include "spin_spin_correlation_sl.h"
#include "main_loop.h"
//#include "tensors.h"
...
main(int argc, char* argv[])
{
    ...
    if (rank==root) {
        ...
        if (NX!=NY) {
            cout<<"Warning: The system is not symmetric!!!"<<endl;
        }
        else {
            if (NX%2==0) {
                cout<<"Warning: The system is not centralized!!!"<<endl;
            }
        }
        
        cout<<"Number of correlators="<<terms<<endl;
        MPI_Barrier(MPI_COMM_WORLD);
    }
    else {
        MPI_Barrier(MPI_COMM_WORLD);
    ...
    //-----begin: setup indices for site tensors and bond tensors---------------------------------------
    for (int i=0; i<N; i++) {
...
//    }
    ...
    //-----end: setup indices for site tensors and bond tensors---------------------------------------
    
    //-----begin: site tensors, share the same form--------------------------------------------------
    for (int x=0; x<NX; x++) {
        for (int y=0; y<NY; y++) {
            int ind=y*NX+x;
            a[ind]=ITensor(s[ind],u[ind],d[ind],l[ind],r[ind]);
            if (rank==0) {
                a[ind]=define_site_tensor(a[ind]);
                //PrintDat(a[ind]);
                a[ind]=a[ind]*scaleup;
                //PrintDat(a[ind]);
            }
    ...
    //-----end: site tensors, share the same form--------------------------------------------------
    
    
    //-----begin: bond tensors-------------------------------------------------------------
    
    //----horizontal bonds bh (NX+1)*NY
    for (int x=0; x<NX+1; x++) {
        for (int y=0; y<NY; y++) {
            int ind=y*(NX+1)+x;
            if (x==0) {
                bh[ind]=ITensor(l[y*NX+x]);
            }
            else if(x==NX){
                bh[ind]=ITensor(r[y*NX+x-1]);
            }
            else {
                bh[ind]=ITensor(l[y*NX+x],r[y*NX+x-1]);
            }
            
            bh[ind]=define_bond_h_tensor(bh[ind], x, y);
            
            //cout<<ind;
            //PrintDat(bh[ind]);
        }
    }
    //----vertical bonds bv NX*(NY+1)
    for (int x=0; x<NX; x++) {
        for (int y=0; y<NY+1; y++) {
            int ind=y*NX+x;
            if (y==0) {
                bv[ind]=ITensor(u[y*NX+x]);
            }
            else if(y==NY){
                bv[ind]=ITensor(d[(y-1)*NX+x]);
            }
            else {
                bv[ind]=ITensor(u[y*NX+x],d[(y-1)*NX+x]);
            }
            
            bv[ind]=define_bond_v_tensor(bv[ind], x, y);
    ...
    //-----begin: combine site tensors and bond tensors to get directly contractable tensors----
    for (int x=0; x<NX; x++) {
        for (int y=0; y<NY; y++) {
            int ind=y*NX+x;
            
            if (x==0 && y==0) {
                a[ind]=a[ind]*bv[ind]*bv[(y+1)*NX+x]*bh[y*(NX+1)+x]*bh[y*(NX+1)+x+1];
            }
            else if(x>0 && y==0){
                a[ind]=a[ind]*bv[ind]*bv[(y+1)*NX+x]*bh[y*(NX+1)+x+1];
                if (x<NX-1) {
                    a[ind]=sort_indices_otho_4(a[ind],0,2,1,3);
                }
                else {
                    a[ind]=sort_indices_otho_3(a[ind],0,2,1);
                }
            }
            else if(x==0 && y>0){
                a[ind]=a[ind]*bv[(y+1)*NX+x]*bh[y*(NX+1)+x]*bh[y*(NX+1)+x+1];
            }
            else {
                a[ind]=a[ind]*bv[(y+1)*NX+x]*bh[y*(NX+1)+x+1];
                if (y<NY-1) {
                    if (x<NX-1) {
                        a[ind]=sort_indices_otho_5(a[ind],0,1,3,2,4);
                    }
                    else {
                        a[ind]=sort_indices_otho_4(a[ind],0,1,3,2);
                    }
                }
            }
    ...
    //-----end: combine site tensors and bond tensors to get directly contractable tensors----

    
    //-----begin: setup tensors IW and W for correlation-----
    if (rank==root) {
        setup_identity_tensors(NX,NY);   //find all IW
...
//        }
        ...
        for (int i=mystart; i<myend; i++) {
            setup_correlator_tensors(i, NX,NY);   //find all W
        }
        MPI_Barrier(MPI_COMM_WORLD);
    }
    //-----end: setup tensors IW and W for correlation-----
    
    
    //-----begin: find all the bra's and ket's and calculate the correlation------------------
    if (rank==root) {
        
        identity_find_bras(IW,NX,NY);  //find all Ibra's
        identity_find_kets(IW,NX,NY);
        
//        for (int aaa=0; aaa<=NY-2; aaa++) {
//            for (int x=0; x<NX; x++) {
//                cout<<x+NX*(NY-2-aaa);
//                PrintDat(Ibra[x+NX*(NY-2-aaa)]);
//            }
//            cout<<"~~~~~~~~~~~~~~~~~~~~~"<<endl;
//            for (int x=0; x<NX; x++) {
//                cout<<x+NX*aaa;
//                PrintDat(Iket[x+NX*aaa]);
//            }
//        }
        ITensor rrr;
        ITensor rrr0;
        ...
        for (int aaa=0; aaa<=0; aaa++) {
            rrr=Ibra[1+NX*(NY-2-aaa)]*Iket[1+NX*aaa];
            //PrintDat(rrr);
            for (int x=2; x<NX; x++) {
                rrr=rrr*(Ibra[x+NX*(NY-2-aaa)]*Iket[x+NX*aaa]);
            //    PrintDat(rrr);
            }
            rrr0=Ibra[0+NX*(NY-2-aaa)]*Iket[0+NX*aaa];
            
            for (int i1=1; i1<=rrr.indices()[0].m(); i1++) {
                for (int i2=1; i2<=rrr.indices()[1].m(); i2++) {
                    _rrr=_rrr+rrr(rrr.indices()[0](i1),rrr.indices()[1](i2))*rrr0(rrr0.indices()[0](i1),rrr0.indices()[1](i2));
                }
            }
    ...
    if (rank==root) {
        ...
        for (int i=mystart; i<myend; i++) {
            ...
            ham_find_kets(WW, i, NX,NY);
            

//            for (int aaa=0; aaa<=NY-2; aaa++) {
//                for (int x=0; x<NX; x++) {
//                    cout<<"i="<<i<<", index="<<x+NX*(NY-2-aaa);
//                    PrintDat(bra[x+NX*(NY-2-aaa)+NX*(NY-1)*i]);
//                }
//                cout<<"~~~~~~~~~~~~~~~~~~~~~"<<endl;
//                for (int x=0; x<NX; x++) {
//                    cout<<"i="<<i<<", index="<<x+NX*aaa;
//                    PrintDat(ket[x+NX*aaa+NX*(NY-1)*i]);
//                }
//                cout<<"<><><><><><><><><><><>"<<endl;
//            }
            //if (i==0) {
            ...
            for (int aaa=0; aaa<=0; aaa++) {
                rrr=bra[1+NX*(NY-2-aaa)+NX*(NY-1)*i]*ket[1+NX*aaa+NX*(NY-1)*i];
                //PrintDat(rrr);
                for (int x=2; x<NX; x++) {
                    rrr=rrr*(bra[x+NX*(NY-2-aaa)+NX*(NY-1)*i]*ket[x+NX*aaa+NX*(NY-1)*i]);
                    //PrintDat(rrr);
                }
                rrr0=bra[0+NX*(NY-2-aaa)+NX*(NY-1)*i]*ket[0+NX*aaa+NX*(NY-1)*i];
                ...
                /////cout<<_rrr<<endl;
                /////PrintDat(rrr*rrr0);
                /////cout<<"{    "<<(i%(NX*NY))%NX<<"    ,   "<<(i%(NX*NY))/NX<<"    ,   "<<_rrr<<"    },"<<endl;
                ////////////printf("{   %d, %d, %.10f    },\n",(i%(NX*NY))%NX,(i%(NX*NY))/NX,_rrr);
    ...
    //-----begin: compress to get the results-----------------
//    if (rank!=root) {
//        for (int i=mystart; i<myend; i++) {
//            MPI_Send(&(rrr(rrr.indices()[0](0))), 1, MPI_DOUBLE, root, i, communicator);
//        }
//        MPI_Barrier(MPI_COMM_WORLD);
//    }
//    else {
...
//        }
//        
//        for (int iii=0; iii<terms; iii++) {
//            MPI_Recv(&(rrresult[iii]), 1, MPI_DOUBLE, rr, iii, communicator, &status);
//        }
//    }
    
    //-----end: compress to get the results-----------------
