...
//dpotrf
//
//Cholesky factorization to check whether a matrix is positive definite

void inline
dpotrf_wrapper(char* uplo,
               LAPACK_INT* n,
               LAPACK_REAL* A,
               LAPACK_INT* info)
    {
...
//// A*x = lambda*B*x
//void inline
//dggev_wrapper(char* jobvl,           //= 'N':  do not compute the left generalized eigenvectors;
//                                     //= 'V':  compute the left generalized eigenvectors.
//              char* jobvr,           //= 'N':  do not compute the right generalized eigenvectors;
//                                     //= 'V':  compute the right generalized eigenvectors.
//              LAPACK_INT* n,        //number of cols of A
//              LAPACK_REAL* A,       //matrix A, on return contains eigenvectors
//              LAPACK_REAL* B,       //matrix B
//              LAPACK_REAL* alphar,       //eigenvalues on return
//              LAPACK_REAL* alphai,
//              LAPACK_REAL* beta,
//              LAPACK_REAL* vl,
//              LAPACK_REAL* vr,
//              LAPACK_INT* info)  //error info
//{
//    LAPACK_INT lwork = max(1,8*(*n));//max(1, 1+6*N+2*N*N);
//    LAPACK_REAL work[lwork];
//    int nevecl = (*jobvl == 'V' ? *n : 1);
//    int nevecr = (*jobvr == 'V' ? *n : 1);
//    
//    dggev_(jobvl,jobvr,n,A,n,B,n,alphar,alphai,beta,vl,&nevecl,vr,&nevecr,work,&lwork,info);
//    //dggev_(jobvl,jobvr,n,A,n,B,n,alphar,alphai,n,beta,n,v,work,&lwork,info);
//    
//}

void inline
dggev_wrapper(char* jobvl,
              char* jobvr,
              LAPACK_INT* n,
              LAPACK_REAL* A,
              LAPACK_REAL* B,
              ...
              LAPACK_INT* info)
{
    LAPACK_INT lwork = max(1,8*(*n));
    LAPACK_REAL work[lwork];
    int nevecl = (*jobvl == 'V' ? *n : 1);
    int nevecr = (*jobvr == 'V' ? *n : 1);
    dggev(jobvl,jobvr,n,A,n,B,n,alphar,alphai,beta,vl,&nevecl,vr,&nevecr,work,&lwork,info);
}
