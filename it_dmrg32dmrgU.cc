...
main(int argc, char* argv[])
{
	...
	const IQMPO H = ExtendedHubbard(model,Opt("U",6.0)& Opt("t1",1)& Opt("t2",0.0)& Opt("V1",0));
	//IQMPO H=ExtendedHubbard(model);
	//readFromFile("Ham_dat",H);
	
	int Nsweeps=4;
	Sweeps sweeps(Nsweeps);
	sweeps.maxm() = 11000,11000,11000,11000;
	sweeps.cutoff() =0.;
	sweeps.niter() = 2;
    ...
    H.doWrite(false);
      cout<<"Writing to disk:"<<endl;
    	writeToFile("psiU6_dat",psi);
	writeToFile("modelU6_dat",model);
	writeToFile("HamU6_dat",H);
